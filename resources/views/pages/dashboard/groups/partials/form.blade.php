                            ...
                            <div class="tab-content" id="languageTabsContent">
                                @foreach (config('app.available_locales', ['en', 'ar']) as $index => $locale)
                                    <div class="tab-pane fade {{ $index == 0 ? 'show active' : '' }}"
                                        ...
                                        <x-forms.input :value="old('name_' . $locale,$isEdit ? $group->getTranslation('name', $locale) : '')" name="name_{{ $locale }}"
                                            placeholder="{{__('name in ') . __($locale)}}"
                                            label="{{ __('name') }} ({{ __($locale) }})" :model="$group"
                                            required />
                                        <x-forms.textarea :value="old('description_' . $locale,$isEdit ? $group->getTranslation('description', $locale) : '')" name="description_{{ $locale }}" 
